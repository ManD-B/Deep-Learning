...
pred2 = data2 %>%
  mutate(pred_QTY = round(410.7761+68.6257*X-73.2499*ITEM_CNT-2.4391*PRICE+93.8410*MAXTEMP+0.0223*SALEDAY))%>%
  summarise(QTY,pred_QTY)
pred3 = data3 %>%
  mutate(pred_QTY = round(407.0016-19.8398*ITEM_CNT+50.1766*MAXTEMP+0.0114*SALEDAY-0.0065*RAIN_DAY))%>%
  summarise(QTY,pred_QTY)
...
acc1 <- pred1 %>%
  ...
  summarise(ACCURACY)
acc1 <- ifelse(acc1$ACCURACY > 100.00, acc1$ACCURACY <- round((abs(pred1$QTY/pred1$pred_QTY * 100)), 2), acc1$ACCURACY <-round((abs(pred1$pred_QTY/pred1$QTY * 100)), 2))
acc1

acc2 <- pred2 %>%
  mutate(ACCURACY = round((abs(pred_QTY/QTY * 100)), 2))%>%
  summarise(ACCURACY)
acc2 <- ifelse(acc2$ACCURACY > 100.00, acc2$ACCURACY <- round((abs(pred2$QTY/pred2$pred_QTY * 100)), 2), acc2$ACCURACY <-round((abs(pred2$pred_QTY/pred2$QTY * 100)), 2))
acc2

acc3 <- pred3 %>%
  mutate(ACCURACY = round((abs(pred_QTY/QTY * 100)), 2))%>%
  summarise(ACCURACY)
acc3 <- ifelse(acc3$ACCURACY > 100.00, acc3$ACCURACY <- round((abs(pred3$QTY/pred3$pred_QTY * 100)), 2), acc3$ACCURACY <-round((abs(pred3$pred_QTY/pred3$QTY * 100)), 2))
acc3
